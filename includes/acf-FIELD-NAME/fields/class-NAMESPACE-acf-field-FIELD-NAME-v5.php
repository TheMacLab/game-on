...
add_filter('acf/fields/taxonomy/query/name=map', 'my_taxonomy_args', 10, 2);
//add_filter('acf/fields/taxonomy/query/name=top', 'my_taxonomy_args', 10, 2);
add_filter('acf/fields/taxonomy/query/name=side', 'my_taxonomy_args', 10, 2);
	...
	function render_field_settings( $field ) {
		...
		*/
		...
		acf_render_field_setting( $field, array(
			...
			'instructions'	=> __('Choose a taxonomy to display.','TEXTDOMAIN'),
			'type'			=> 'select',
			'name'			=> 'taxonomy',
	...
	*/
	...
	function render_field( $field ) {
		...
		*/
		...
		$terms = get_terms( $taxonomy, $term_args_parent);
	   ...
	   ?>
		...
		<div class="acf-taxonomy-field" data-save="0" data-type="select" data-taxonomy="<?php echo $taxonomy; ?>">
		<div class="acf-actions -hover">
		<a href="#" class="acf-icon -plus acf-js-tooltip small" data-name="add" title="<?php echo esc_attr($taxonomy_details->labels->add_new_item); ?>"></a>
		</div>
		...
		<select class='select2_taxonomy' id="<?php echo esc_attr($field['id']); ?>" name="<?php echo esc_attr($field['name']); ?>" data-ui="1"  data-multiple="0" data-placeholder="Select" data-allow_null="0" onchange="updateinput();" >
		<?php
		...
		foreach ( $terms as $term ) {
				...
				$terms_child = get_terms( $taxonomy, $term_args_child);
				if ( is_a( $terms_child, 'WP_Error' ) ) {
					$terms_child = array();
				}
				$child_object_terms = wp_get_object_terms( $post->ID, $taxonomy, array('fields'=>'ids'));
				if ( is_a( $object_terms, 'WP_Error' ) ) {
