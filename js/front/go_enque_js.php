...
function go_scripts () {
    ...
    wp_register_script( 'go_wp_media', get_site_url(null, 'wp-admin/css/media.css'), null, 4.09 );
	...
	wp_enqueue_style( 'dashicons' );
		...
		wp_register_script( 'go_tasks', plugin_dir_url( __FILE__ ).'min/go_tasks-min.js', null, 4.09 );

		//COMBINED FILE
		wp_register_script( 'go_frontend-min', plugin_dir_url( __FILE__ ).'min/go_frontend-min.js', array('jquery'), 4.09, false);

		//PAGE SPECIFIC
		wp_register_script('go_map-min', plugins_url('min/go_map-min.js', __FILE__), array('jquery'), 4.09, true);
	 ...
	 */
        ...
        if ($is_admin){
            wp_register_script('go_admin_notification_listener', plugins_url('min/go_admin_notifications-min.js', __FILE__), array('jquery'), 4.09, true);
