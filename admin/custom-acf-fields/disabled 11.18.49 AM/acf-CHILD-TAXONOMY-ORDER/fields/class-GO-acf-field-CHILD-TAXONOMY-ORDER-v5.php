...
//Fix new term button--refresh terms on add
...
if( !class_exists('GO_acf_field_CHILD_TAXONOMY_ORDER') ) :
		...
		//add_action('wp_ajax_acf/fields/taxonomy/add_term_GO',		array($this, 'ajax_add_term_GO'));
	
	


class GO_acf_field_CHILD_TAXONOMY_ORDER extends acf_field {
	...
	*/
	...
	function __construct( $settings ) {
		...
		*/
		...
		$this->name = 'CHILD_TAXONOMY_ORDER';
		...
		*/
		...
		$this->label = __('CHILD TAXONOMY ORDER', 'TEXTDOMAIN');
	...
	*/
	...
	function render_field( $field ) {
		...
		*/
		...
		$term_name = get_cat_name( $field_value );
	   ...
	   ?>
    	...
    	echo "<option value='".esc_attr($field['value'])."' selected='selected' >".$term_name."</option>";
    	...
    	foreach ( $terms as $term ) {
	           ...
	           $term_name = $term->name;
				...
				foreach ( $terms_child as $term_child ) {
		            ...
		            $children[] = array(
						'id'	=> $child_term_ID,
						'text'	=> $child_term_name,
						);
		        }
	        $dataPoints[] = array("text"=>$term_name,"children"=>$children,);
	    }	
	    echo "findthis";
	    echo $dataPoints;
	    debug_to_console( $dataPoints );
	    debug_to_console( "Hi" );
	    var_dump($dataPoints);
	    echo '<pre>';
			print_r( $dataPoints );
...
// initialize
new GO_acf_field_CHILD_TAXONOMY_ORDER( $this->settings );
...
//Filter parent only taxonomy for new taxonomy terms
