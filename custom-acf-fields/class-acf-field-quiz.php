...
class acf_field_quiz extends acf_field {
	...
	*/
	...
	function render_field( $field ) {
		...
		$meta_id = 'stages_'.$stage_num.'_quiz';
		...
		<table  class='go_test_field_table go_test_field_table_<?php echo $ttc; ?>'>
			<?php
			if ( ! empty( $test_field_block_count ) && ! empty( $test_field_input_array ) ) {
				for ( $i = 0; $i < $test_field_block_count; $i++ ) {
					if ( ! empty( $test_field_input_array[ $i ][0] ) ) {
						echo "
						<tr  class='go_test_field_input_row_{$ttc} go_test_field_input_row go_test_field_input_row_{$ttc}_{$i}'>
							<td>
								<select  class='go_test_field_input_select_{$ttc} go_test_field_select_{$ttc}_{$i}' name='go_test_field_select_{$ttc}[]' onchange='update_checkbox_type_{$ttc}(this);'>
				...
				echo "
					<tr class='go_test_field_input_row_{$ttc} go_test_field_input_row go_test_field_input_row_{$ttc}_0'>
						<td>
							<select class='go_test_field_input_select_{$ttc}' name='go_test_field_select_{$ttc}[] go_test_field_select_{$ttc}_0' onchange='update_checkbox_type_{$ttc}(this);'>
			...
			<tr>
				<td>
					<input class='go_button_add_field go_test_field_add_block_button go_test_field_add_block_button_<?php echo $ttc; ?>' value='Add Question' type='button' onclick='add_block_<?php echo $ttc; ?>(this);' />
					<?php 
					if ( ! empty( $test_field_block_count ) ) {
						echo "<input class='go_test_field_block_count_{$ttc}' name='go_test_field_block_count_{$ttc}' type='hidden' value='{$test_field_block_count}' />";
					} else {
						echo "<input class='go_test_field_block_count_{$ttc}' name='go_test_field_block_count_{$ttc}' type='hidden' value='1' />";
					}
					?>
				</td>
			</tr>
