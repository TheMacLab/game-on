...
function task_change_stage(){
	...
	$go_table_ind = $wpdb->prefix.'go';
	$task_count = $wpdb->get_var("select `count` from ".$go_table_ind." where post_id = $task_id and uid = $user_id");
		...
		if($repeat_button != 'on'){
			...
			if($undo == 'true' || $undo === true){
				if ($task_count > 0) {
					...
					go_add_post($user_id, $task_id, ($status-1), -$points_array[$status-1], -$currency_array[$status-1], $page_id, $repeat_button, 0);
				}
			} else {
