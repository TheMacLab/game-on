...
jQuery("div.dataTables_filter input").unbind(),
//search on clear with 'x'
document.querySelector("#go_clipboard_activity_datatable_filter input").onsearch=function(e){Activity.search(this.value).draw()})}function go_clipboard_update(){go_save_clipboard_filters();
//*******************//
//GET CURRENT TAB
//*******************//
...
//d.user_id = jQuery('#go_stats_hidden_input').val();
e.date=jQuery("#datepicker_clipboard").val(),e.section=jQuery("#go_clipboard_user_go_sections_select").val(),e.group=jQuery("#go_clipboard_user_go_groups_select").val(),e.badge=jQuery("#go_clipboard_go_badges_select").val(),e.unmatched=document.getElementById("go_unmatched_toggle").checked,e.tasks=jQuery("#go_task_select").val();for(var a=0,t=data.columns.length;a<t;a++)data.columns[a].search.value||delete data.columns[a].search,!0===data.columns[a].searchable&&delete data.columns[a].searchable,!0===data.columns[a].orderable&&delete data.columns[a].orderable,data.columns[a].data===data.columns[a].name&&delete data.columns[a].name;delete data.search.regex}},deferRender:!0,bPaginate:!0,
//colReorder: true,
...
if(console.log("6"+l),1!=l&&"true"!=l||jQuery("#go_unmatched_toggle").prop("checked",!0),jQuery("#go_clipboard_user_go_sections_select").select2({ajax:{url:ajaxurl,// AJAX URL is predefined in WordPress admin
dataType:"json",delay:400,// delay in ms while typing when to perform a AJAX search
data:function(e){return{q:e.term,// search query
action:"go_make_taxonomy_dropdown_ajax",// AJAX action for admin-ajax.php
taxonomy:"user_go_sections"}},processResults:function(e){console.log("here: "+e);var t=[];return e&&
// data is the array of arrays, and each of them contains ID and the Label of the option
jQuery.each(e,function(e,a){// do not forget that "index" is just auto incremented value
t.push({id:a[0],text:a[1]})}),jQuery("#go_clipboard_user_go_sections_select").select2("destroy"),jQuery("#go_clipboard_user_go_sections_select").children().remove(),jQuery("#go_clipboard_user_go_sections_select").select2({data:t,placeholder:"Show All",allowClear:!0}).val(a).trigger("change"),jQuery("#go_clipboard_user_go_sections_select").select2("open"),{results:t}},cache:!0},minimumInputLength:0,// the minimum of symbols to input before perform a search
...
data:function(e){return{q:e.term,// search query
action:"go_make_taxonomy_dropdown_ajax",// AJAX action for admin-ajax.php
taxonomy:"go_badges"}},processResults:function(e){console.log("search results: "+e);var t=[];return e&&
// data is the array of arrays, and each of them contains ID and the Label of the option
jQuery.each(e,function(e,a){// do not forget that "index" is just auto incremented value
t.push({id:a[0],text:a[1]})}),jQuery("#go_clipboard_go_badges_select").select2("destroy"),jQuery("#go_clipboard_go_badges_select").children().remove(),jQuery("#go_clipboard_go_badges_select").select2({data:t,placeholder:"Show All",allowClear:!0}).val(o).trigger("change"),jQuery("#go_clipboard_go_badges_select").select2("open"),{results:t}},cache:!1},minimumInputLength:0,// the minimum of symbols to input before perform a search
multiple:!1,placeholder:"Show All",allowClear:!0}),null!=o&&"null"!=o){console.log("setB");
// Fetch the preselected item, and add to the control
var _=jQuery("#go_clipboard_go_badges_select"),i=new Option(t,o,!0,!0);
// create the option and append to Select2
_.append(i).trigger("change")}
