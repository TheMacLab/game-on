...
//For the Timer (v4)
function getTimeRemaining(e){var o=Date.parse(e)-Date.parse(new Date),r=Math.floor(o/1e3%60),t=Math.floor(o/1e3/60%60),s=Math.floor(o/36e5%24),a;return{total:o,days:Math.floor(o/864e5),hours:s,minutes:t,seconds:r}}
//Initializes the new timer (v4)
function initializeClock(e,r){function o(){var e=getTimeRemaining(r),o;(e.days=Math.max(0,e.days),s.innerHTML=e.days,e.hours=Math.max(0,e.hours),a.innerHTML=("0"+e.hours).slice(-2),e.minutes=Math.max(0,e.minutes),_.innerHTML=("0"+e.minutes).slice(-2),e.seconds=Math.max(0,e.seconds),n.innerHTML=("0"+e.seconds).slice(-2),e.total=0)&&(clearInterval(g),new Audio(PluginDir.url+"media/airhorn.mp3").play())}var t=document.getElementById(e),s=t.querySelector(".days"),a=t.querySelector(".hours"),_=t.querySelector(".minutes"),n=t.querySelector(".seconds");o();var i=getTimeRemaining(r),u=i.total;if(console.log(i.total),0<u)var g=setInterval(o,1e3)}function go_timer_abandon(){var e=go_task_data.redirectURL;
...
//alert(json_status);
if("101"===Number.parseInt(o.json_status)){console.log(101),jQuery("#go_stage_error_msg").show();var r="Server Error.";jQuery("#go_stage_error_msg").text()!=r?jQuery("#go_stage_error_msg").text(r):flash_error_msg("#go_stage_error_msg")}else if(302===Number.parseInt(o.json_status))console.log(302),window.location=o.location;else if("refresh"==o.json_status)location.reload();else if("bad_password"==o.json_status){jQuery("#go_stage_error_msg").show();var r="Invalid password.";jQuery("#go_stage_error_msg").text()!=r?jQuery("#go_stage_error_msg").text(r):flash_error_msg("#go_stage_error_msg")}else{if("undo"==o.button_type)jQuery("#go_wrapper div").last().hide(),jQuery("#go_wrapper > div").slice(-3).hide("slow",function(){jQuery(this).remove()});else if("undo_last"==o.button_type)jQuery("#go_wrapper div").last().hide(),jQuery("#go_wrapper > div").slice(-2).hide("slow",function(){jQuery(this).remove()});else if("continue"==o.button_type)jQuery("#go_wrapper > div").slice(-1).hide("slow",function(){jQuery(this).remove()});else if("complete"==o.button_type)jQuery("#go_wrapper > div").slice(-1).hide("slow",function(){jQuery(this).remove()});else if("show_bonus"==o.button_type)jQuery("#go_buttons").remove(),
//remove active class to checks and buttons
jQuery(".go_checks_and_buttons").removeClass("active");else if("continue_bonus"==o.button_type)jQuery("#go_wrapper > div").slice(-1).hide("slow",function(){jQuery(this).remove()});else if("complete_bonus"==o.button_type)jQuery("#go_wrapper > div").slice(-1).hide("slow",function(){jQuery(this).remove()});else if("undo_bonus"==o.button_type)jQuery("#go_wrapper > div").slice(-2).hide("slow",function(){jQuery(this).remove()});else if("undo_last_bonus"==o.button_type)jQuery("#go_wrapper > div").slice(-1).hide("slow",function(){jQuery(this).remove()});else if("abandon_bonus"==o.button_type)jQuery("#go_wrapper > div").slice(-3).remove();else if("abandon"==o.button_type)window.location=o.redirect;else if("timer"==o.button_type){
...
//console.log(res.html);
jQuery("go_hidden_mce").remove(),go_append(o),
...
var s=_[t].children[1].children[0].type;o.push(s);
...
var e=go_task_data.status,o=go_task_data.currency;go_make_clickable(),jQuery(".go_stage_message").show(),jQuery(".go_checks_and_buttons").show("slow"),
