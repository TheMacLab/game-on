function go_update_bonus_loot(){jQuery("#go_bonus_loot").html('<span class="go_loading"></span>');var e=GO_EVERY_PAGE_DATA.nonces.go_update_bonus_loot,r=go_task_data.ID;
//alert (post_id);
jQuery.ajax({type:"post",url:MyAjax.ajaxurl,data:{_ajax_nonce:e,action:"go_update_bonus_loot",post_id:r},success:function(e){console.log("Bonus Loot"),console.log(e),jQuery("#go_bonus_loot").remove(),jQuery("#go_wrapper").append(e)}})}
//For the Timer (v4)
function getTimeRemaining(e){var r=Date.parse(e)-Date.parse(new Date),o=Math.floor(r/1e3%60),t=Math.floor(r/1e3/60%60),s=Math.floor(r/36e5%24),a;return{total:r,days:Math.floor(r/864e5),hours:s,minutes:t,seconds:o}}
//Initializes the new timer (v4)
function initializeClock(e,o){function r(){var e=getTimeRemaining(o),r;(e.days=Math.max(0,e.days),s.innerHTML=e.days,e.hours=Math.max(0,e.hours),a.innerHTML=("0"+e.hours).slice(-2),e.minutes=Math.max(0,e.minutes),_.innerHTML=("0"+e.minutes).slice(-2),e.seconds=Math.max(0,e.seconds),n.innerHTML=("0"+e.seconds).slice(-2),e.total=0)&&(clearInterval(g),new Audio(PluginDir.url+"media/airhorn.mp3").play())}var t=document.getElementById(e),s=t.querySelector(".days"),a=t.querySelector(".hours"),_=t.querySelector(".minutes"),n=t.querySelector(".seconds");r();var i=getTimeRemaining(o),u=i.total;if(console.log(i.total),0<u)var g=setInterval(r,1e3)}function go_timer_abandon(){var e=go_task_data.redirectURL;
//window.location = $homeURL;
window.location=e}function flash_error_msg(e){var r=jQuery(e).css("background-color");void 0===typeof r&&(r="white"),jQuery(e).animate({color:r},200,function(){jQuery(e).animate({color:"red"},200)})}
...
function go_disable_loading(){console.log("oneclick"),jQuery(".go_loading").remove(),jQuery("#go_button").off().one("click",function(e){task_stage_check_input(this)}),jQuery("#go_back_button").off().one("click",function(e){task_stage_check_input(this)}),jQuery("#go_bonus_button").off().one("click",function(e){go_update_bonus_loot(this)}),
...
// Animation complete
Vids_Fit_and_Box(),go_make_clickable(),go_disable_loading(),go_mce()})}
...
go_admin_view:e},success:function(e){location.reload()},error:function(e){console.log(e),console.log("fail")}})}function go_quiz_check_answers(e,a){
//if ( jQuery( ".go_test_list" ).length != 0) {
var _=jQuery(".go_test_list"),n=_.length;if(jQuery(".go_test_list :checked").length>=n){
//var test_list = jQuery( ".go_test_list" );
//var list_size = test_list.length;
var r=[];if(1<jQuery(".go_test_list").length){for(var o=[],t=0;t<n;t++){
// figure out the type of each test
var s=_[t].children[1].children[0].type;r.push(s);
// get the checked inputs of each test
var i="#"+_[t].id+" :checked",u=jQuery(i);if("radio"==s)
// push indiviudal answers to the choice_array
null!=u[0]&&o.push(u[0].value);else if("checkbox"==s){for(var g=[],l=0;l<u.length;l++)g.push(u[l].value);var c=g.join("### ");o.push(c)}}var d=o.join("#### "),y=r.join("### ")}else{var j=jQuery(".go_test_list li input:checked"),y;if("radio"==(y=jQuery(".go_test_list li input").first().attr("type")))var d=j[0].value;else if("checkbox"==y){for(var d=[],l=0;l<j.length;l++)d.push(j[l].value);d=d.join("### ")}}}jQuery.ajax({type:"POST",data:{_ajax_nonce:go_task_data.go_unlock_stage,action:"go_unlock_stage",task_id:go_task_data.ID,user_id:go_task_data.userID,list_size:n,chosen_answer:d,type:y,status:e},success:function(e){if("refresh"==e)location.reload();else{if(1==e)return jQuery(".go_test_container").hide("slow"),jQuery("#test_failure_msg").hide("slow"),jQuery(".go_test_submit_div").hide("slow"),jQuery(".go_wrong_answer_marker").hide(),jQuery("#go_stage_error_msg").hide(),task_stage_change(a),0;//return a mod of 0
if(0==e)
//go_disable_loading();
...
for(var r=e.split(", "),o=0;o<_.length;o++){var t="#"+_[o].id;-1===jQuery.inArray(t,r)?(jQuery(t+" .go_wrong_answer_marker").is(":visible")&&jQuery(t+" .go_wrong_answer_marker").hide(),jQuery(t+" .go_correct_answer_marker").is(":visible")||jQuery(t+" .go_correct_answer_marker").show()):(jQuery(t+" .go_correct_answer_marker").is(":visible")&&jQuery(t+" .go_correct_answer_marker").hide(),jQuery(t+" .go_wrong_answer_marker").is(":visible")||jQuery(t+" .go_wrong_answer_marker").show())}var s;
//go_disable_loading();
...
var e=go_task_data.status,r=go_task_data.currency;go_make_clickable(),jQuery(".go_stage_message").show(),jQuery(".go_checks_and_buttons").show("slow"),
