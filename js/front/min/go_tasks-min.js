...
var r="";void 0!==jQuery(e).attr("status")&&(r=jQuery(e).attr("status"));var t="";void 0!==jQuery(e).attr("check_type")&&(t=jQuery(e).attr("check_type"));var s=jQuery("#go_admin_bar_progress_bar").css("background-color"),a=jQuery("#go_result").attr("value");if("blog"==t&&"undo_last_bonus"!=o){
//result = tinyMCE.activeEditor.getContent();
a=go_get_tinymce_content_check();var _=jQuery("#go_result_title_check").val(),n=jQuery("#go_result_title").attr("blog_post_id")}else var _=null;jQuery.ajax({type:"POST",data:{_ajax_nonce:go_task_data.go_task_change_stage,action:"go_task_change_stage",post_id:go_task_data.ID,user_id:go_task_data.userID,status:r,button_type:o,check_type:t,result:a,result_title:_,blog_post_id:n},success:function(e){console.log("success");
//console.log(raw);
...
jQuery(".go_checks_and_buttons").removeClass("active");else if("continue_bonus"==o.button_type||"complete_bonus"==o.button_type)jQuery("#go_wrapper > div").slice(-1).hide("slow",function(){jQuery(this).remove()});else if("undo_bonus"==o.button_type)jQuery("#go_wrapper > div").slice(-2).hide("slow",function(){jQuery(this).remove()});else if("undo_last_bonus"==o.button_type)jQuery("#go_wrapper > div").slice(-1).hide("slow",function(){jQuery(this).remove()});else if("abandon_bonus"==o.button_type)jQuery("#go_wrapper > div").slice(-3).remove();else if("abandon"==o.button_type)window.location=o.redirect;else if("timer"==o.button_type){
...
var i="#"+_[t].id+" :checked",g=jQuery(i);if("radio"==s)
