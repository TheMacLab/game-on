 ...
 $new_rank_array= array(array($new_rank, $rank_points),array($next_rank, $next_rank_points));
  update_user_meta($uids,'go_rank', $new_rank_array );
     ...
     <div>   <table style="width:250px;" id="ranks_table" class="widefat">
        ...
        <th style="width:125px;"><?php echo get_option('go_points_name'); ?></th><th></th><th></th><tbody id="table_rows">
        <?php  
	...
	function go_save_rank_trigger(level){
		...
		ajaxurl = '<?= get_site_url() ?>/wp-admin/admin-ajax.php';
		jQuery.ajax({
		type: "post",url: ajaxurl,data: { action: 'go_save_rank_trigger', rank: level, trigger : jQuery('#go_rank_trigger_'+level_id).val() }
	});
